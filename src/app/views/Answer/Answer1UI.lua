...
--Create root
local Answer1UI = class("Answer1UI", cc.load("mvc").ViewBase, function()
    root = cc.Node:create()
    root:setName("uiRoot")
    return root
end)
...
--Create Cocos Studio UI
    local function createCocosStudioUI()
        ...
        label_Aside:setString(Aside_Answer["Aside_Answer1"])
...
--构造函数,在create中执行
function Answer1UI:ctor()
        initUI()
end

function Answer1UI:getUIItem(args)
