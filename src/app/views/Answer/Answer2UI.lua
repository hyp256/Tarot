...
--Create root
local Answer2UI = class("Answer2UI", cc.load("mvc").ViewBase, function()
    root = cc.Node:create()
    root:setName("uiRoot")
    return root
end)
...
--Create Cocos Studio UI
    local function createCocosStudioUI()
        ...
        label_Aside:setString(Aside_Answer["Aside_Answer2"])
...
--构造函数,在create中执行
function Answer2UI:ctor()
        initUI()
end

function Answer2UI:getUIItem(args)
