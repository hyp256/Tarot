...
--Create root
local Answer4UI = class("Answer4UI", cc.load("mvc").ViewBase, function()
    root = cc.Node:create()
    root:setName("uiRoot")
    return root
end)
...
--Create Cocos Studio UI
    local function createCocosStudioUI()
        ...
        label_Aside:setString(Aside_Answer["Aside_Answer4"])
...
--构造函数,在create中执行
function Answer4UI:ctor()
        initUI()
end

function Answer4UI:getUIItem(args)
