...
function PlayphoneScene:setupTestMenu()
    ...
    print("=============================================4")
    self.status =
    cc.Label:createWithSystemFont("", "Arial", 32)
            :move(display.cx, 100)
            :addTo(self)
...
--[[
    ...
    local playphoneDone = function()
        sdkbox.PluginAchievement:unlock(SOLDIER_ACHIEVEMENT_ID)
        sdkbox.PluginLeaderboard:getLeaderboard(LEVEL_LEADER_BOARD_ID)
        print("=============================================3")
    end

    cc.Director:getInstance():getScheduler():scheduleScriptFunc(playphoneDone,2,false)
end
